...
theorem Typed.preservation {Γ : Ctx} {A m m'} :
    Γ ⊢ m : A -> m ~> m' -> Γ ⊢ m' : A := by
  ...
  case tup tyS _ _ _ ihm ihn =>
    ...
    case tup_N st =>
      ...
      . assumption
      . apply ihn st
  case prj Γ A B C _ _ iC tyC tym tyn ihC ihm ihn =>
    have ⟨_, wf, tyS⟩ := tyC.ctx_inv
    have ⟨_, _, tyB, _⟩ := tyS.sig_inv
    ...
    case prj_A C' st =>
      replace ihC := ihC st
      have wf' := tyn.toWf
      have tyC' : (.sig A B).[shift 2] :: B :: A :: Γ ⊢ C'.[ren (upren (.+2))] :
                  (.ty iC).[ren (upren (.+2))] := by
      ...
      have typ : B :: A :: Γ ⊢ .tup (.var 1) (.var 0) : (.sig A B).[shift 2] := by
    ...
    case prj_elim m1 m2 =>
      have ⟨tym1, tym2⟩ := tym.tup_inv
      rw[show C.[.tup m1 m2/]
            = C.[.tup (.var 1) (.var 0) .: shift 2].[m2,m1/] by asimp]
  ...
  case rfl tym ihm =>
    ...
    case rfl_M st =>
      ...
      . constructor
        apply ihm st
      . constructor <;> assumption
  case rw Γ A B m n a b i tyA tym tyn ihA ihm ihn =>
    have wf := tym.toWf
    have ⟨_, tyI⟩ := tyn.validity
    have ⟨_, tya, tyb, _⟩ := tyI.idn_inv
    have ⟨_, _⟩ := tya.validity
    cases st
    case rw_A A' st =>
      have : Γ ⊢ A'.[.rfl a, a/] : .ty i := by
        rw[show .ty i = (.ty i).[.rfl a,a/] by asimp]
      ...
      have : Γ ⊢ A.[n,b/] : .ty i := by
        rw[show .ty i = (.ty i).[n,b/] by asimp]
        apply Typed.substitution
        . assumption
        . apply AgreeSubst.intro
          asimp; assumption
    ...
    case rw_elim c =>
      ...
      have : Γ ⊢ A.[.rfl c,b/] : .ty i := by
        rw[show .ty i = (.ty i).[.rfl c,b/] by asimp]
        apply Typed.substitution
        . assumption
        . apply AgreeSubst.intro
          asimp; assumption
...
theorem Typed.preservation' {Γ : Ctx} {A m n} :
    Γ ⊢ m : A -> m ~>* n -> Γ ⊢ n : A := by
  intro ty rd
