...
--   have ⟨⟨H1, c'⟩, ⟨rd, h⟩⟩ := sn.wn; clear sn
--   have ⟨b', rs, _⟩ := rs.preservationX rd
--   match rs.progessX h with
--   | .inl ⟨Hx, x, st⟩  =>
--     left; existsi Hx, x; apply Step.intro rd h st
--   | .inr ⟨l, e⟩ =>
--     subst_vars
--     right; exists H1, l
...
lemma Resolved.normal_poised {H1 H2 H3 : Heap Srt} {a b c A} :
...
--     [] ;; [] ;; H ⊢ a ▷ b ◁ c : A -> Normal Step01 (H, c) ->
--     (∃ H' c', Step2 (H, c) (H', c')) ∨ (∃ l, c = .ptr l)  := by
--   generalize e1: [] = Γ
--   generalize e2: [] = Δ
--   intro ⟨er, rs⟩ norm; induction er generalizing H c
--   case var hs => subst_vars; cases hs
--   case lam_im erm ih =>
--     subst_vars; clear ih
--     cases rs
