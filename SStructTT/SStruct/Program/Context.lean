...
@[simp]def logical : Ctx Srt -> Logical.Ctx Srt
  | [] => []
  | ⟨A, _, _⟩ :: (Δ : Ctx Srt) => A :: Δ.logical
...
lemma logical_length {Δ : Ctx Srt} : Δ.logical.length = Δ.length := by
  induction Δ <;> aesop
...
omit ord in
@[simp]lemma Implicit.logical (Δ : Ctx Srt) :
    Δ.toImplicit.logical = Δ.logical := by
  induction Δ
  case nil => simp
  case cons ih =>
    simp; rw[<-ih]; rfl
...
omit ord in
lemma Has.logical {Δ : Ctx Srt} {A x s} :
    Has Δ x s A -> Logical.Has Δ.logical x A := by
...
lemma Merge.logical {Δ1 Δ2 Δ3 : Ctx Srt} :
    Merge Δ1 Δ2 Δ3 -> Δ3.logical = Δ1.logical := by
