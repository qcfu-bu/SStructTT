...
lemma AgreeRen.wf_cons {Γ Γ' : Ctx Srt} {A ξ} :
    ...
    (∀ Γ' ξ, AgreeRen ξ Γ Γ' → ∃ s i, Γ' ⊢ A⟨ξ⟩ : .srt s i) ->
    Γ' ⊢ := by
  intro
  | cons ty agr =>
    intro wf h1 h2
...
lemma Typed.renaming {Γ Γ' : Ctx Srt} {A m ξ} :
  ...
  case prj Γ A B C m n r s sC iC tyC tym tyn ihC ihm ihn =>
    ...
    asimp at ihC
    asimp at ihm
    rw[show C[.tup (.var 1) (.var 0) r s .: shift >> shift >> Tm.var_Tm]⟨up_ren (up_ren ξ)⟩
          = (C⟨up_ren ξ⟩)[.tup (.var 1) (.var 0) r s .: shift >> shift >> Tm.var_Tm] by asimp] at ihn
  ...
  case bot ih =>
    constructor;
    apply ih; assumption
  case exf A _ _ _ tyA _ ihA ihm =>
    have ⟨_, _, _, tyb⟩ := tyA.ctx_inv
    replace ihA := ihA (agr.cons tyb); asimp at ihA
    replace ihm := ihm agr; asimp at ihm
