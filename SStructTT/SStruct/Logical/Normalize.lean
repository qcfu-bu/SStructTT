...
@[simp]def interp_ctx : Ctx Srt -> ECC.Ctx
  | [] => []
...
lemma interp_red {m n : Tm Srt} :
    Star Step m n -> Star ECC.Step [| m |] [| n |] := by
...
lemma interp_conv {m n : Tm Srt} :
    Conv Step m n -> Conv ECC.Step [| m |] [| n |] := by
...
lemma interp_sn {m : Tm Srt} :
    SN ECC.Step [| m |] -> SN Step m := by
  intro sn; apply interp_sn' <;> aesop

variable [ord : SrtOrder Srt]

theorem Typed.toECC {Γ : Ctx Srt} {A m} :
    Typed Γ m A -> ECC.Typed [| Γ |]* [| m |] [| A |] := by
  intro ty; induction ty using
    @Typed.rec _ ord (motive_2 := fun Γ _ => ECC.Wf [| Γ |]*)
