...
theorem Extract.validity {Δ : Ctx Srt} {A m m'} :
    Δ ⊢ m ▷ m' :: A -> ∃ s i, Δ.logical ⊢ A : .srt s i := by
  intro erm; apply erm.toProgram.validity

lemma Extract.lam_im_inv' {Δ : Ctx Srt} {A T m m' s} :
...
lemma Extract.lam_ex_inv' {Δ : Ctx Srt} {A T m m' s} :
...
lemma Extract.tup_im_inv' {Δ : Ctx Srt} {T m m' n n' s} :
...
lemma Extract.tup_ex_inv' {Δ : Ctx Srt} {T m m' n n' s} :
...
lemma Extract.lam_im_inv {Δ : Ctx Srt} {A A' B m m' s s'} :
  ...
  have ⟨s, i, tyP⟩ := er.toLogical.validity
  ...
  apply Extract.conv_ctx eqA tyA'
  apply Extract.conv eqB.sym erm tyB

lemma Extract.lam_ex_inv {Δ : Ctx Srt} {A A' B m m' s s'} :
...
lemma Extract.tup_ex_inv {Δ : Ctx Srt} {A B m m' n n' s s'} :
  ...
  replace erm := Extract.conv eqA.sym erm tyA
  rw[mrg.logical] at tyB
  replace tyB := tyB.subst erm.toLogical; asimp at tyB
...
lemma Extract.tup_preimage {B t : SStruct.Tm Srt} {m' n' s} :
    [] ⊢ t ▷ .tup m' n' s :: B ->
...
lemma Extract.tt_preimage {B t : SStruct.Tm Srt} :
    [] ⊢ t ▷ .tt :: B -> t ~>>* .tt := by
  generalize e1: [] = Δ
...
lemma Extract.ff_preimage {B t : SStruct.Tm Srt} :
    [] ⊢ t ▷ .ff :: B -> t ~>>* .ff := by
  generalize e1: [] = Δ
