...
inductive AgreeSubst :
  (Var -> SStruct.Tm Srt) ->
  (Var -> Extraction.Tm Srt) ->
  Program.Ctx Srt ->
  Program.Ctx Srt -> Prop
where
  | nil {σ σ'} :
    AgreeSubst σ σ' [] []
  | cons {Δ Δ' : Ctx Srt} {A s i σ σ'} r :
    Δ.logical ⊢ A : .srt s i ->
    AgreeSubst σ σ' Δ Δ' ->
    AgreeSubst (⇑σ) (⇑σ') (A :⟨r, s⟩ Δ) (A[σ] :⟨r, s⟩ Δ')
  ...
  | conv {Δ Δ' : Ctx Srt} {A B r s i σ σ'} :
    A === B ->
    Δ.logical ⊢ B : .srt s i ->
    Δ'.logical ⊢ B⟨↑⟩[σ] : .srt s i ->
    AgreeSubst σ σ' (A :⟨r, s⟩ Δ) Δ' ->
    AgreeSubst σ σ' (B :⟨r, s⟩ Δ) Δ'

lemma AgreeSubst.toProgram {Δ Δ' : Ctx Srt} {σ σ'} :
    Extraction.AgreeSubst σ σ' Δ Δ' -> Program.AgreeSubst σ Δ Δ' := by
  intro agr
...
lemma AgreeSubst.toLogical {Δ Δ' : Ctx Srt} {σ σ'} :
    Extraction.AgreeSubst σ σ' Δ Δ' -> Logical.AgreeSubst σ Δ.logical Δ'.logical := by
  intro agr; apply agr.toProgram.toLogical

@[aesop safe (rule_sets := [subst])]
lemma AgreeSubst.refl {Δ : Ctx Srt} : Δ ⊢ -> AgreeSubst SStruct.Tm.var_Tm Extraction.Tm.var_Tm Δ Δ := by
...
lemma AgreeSubst.has {Δ Δ' : Ctx Srt} {A x s σ σ'} :
  ...
  case cons A s i σ σ' r tyA agr ih =>
    ...
    | im =>
      ...
      rw[show A[σ >> ⟨↑⟩] = A[σ]⟨↑⟩ by asimp]
      apply Extract.eweaken_im <;> try first | rfl | assumption
      apply ih <;> assumption
  case intro_im ih =>
    rcases hs with _ | @⟨_, A, x, s, hs⟩; asimp
    apply ih <;> assumption
  ...
  case conv r _ _  σ σ' eq tyB tyB' agr ih =>
    ...
    | ex =>
      rcases hs with ⟨h⟩
      apply Extract.conv
      . apply Logical.Conv.subst _ (Logical.Conv.ren _ eq)
...
lemma AgreeSubst.wf {Δ Δ' : Ctx Srt} {σ σ'} :
  ...
  intro agr; apply agr.toProgram.wf

lemma Extract.substitution {Δ Δ' : Ctx Srt} {A m m' σ σ'} :
    Δ ⊢ m ▷ m' :: A -> AgreeSubst σ σ' Δ Δ' -> Δ' ⊢ m[σ] ▷ m'[σ'] :: A[σ] := by
  intro ty agr; induction ty generalizing Δ' σ σ' <;> (try asimp)
  case var wf hs =>
    apply agr.has (agr.wf wf) hs
  case lam_im lw tyA erm ihm =>
    apply Extract.lam_im
  ...
  case app_ex mrg erm ern ihm ihn =>
    ...
    specialize ihn agr2
    have er := Extract.app_ex mrg ihm ihn
    asimp at er; assumption
  case tup_im B m m' n s i tyS tym ern ihn =>
    ...
    rw[show B[m[σ] .: σ] = B[⇑σ][m[σ]/] by asimp] at ern
    have er := Extract.tup_im tyS tym ern; assumption
  case tup_ex B m m' n n' s i mrg tyS erm ern ihm ihn =>
    ...
    rw[show B[m[σ] .: σ] = B[⇑σ][m[σ]/] by asimp] at ern
    have er := Extract.tup_ex mrg tyS erm ern; assumption
  case prj_im A B C m m' n n' s sA sB sC iC mrg tyC erm ern ihm ihn =>
    ...
    rw[show C[m[σ] .: σ] = C[⇑σ][m[σ]/] by asimp]
    apply Extract.prj_im <;> (first | assumption | (asimp; renamify; assumption))
  ...
  case ite A m m' n1 n1' n2 n2' s i mrg tyA erm ern1 ern2 ihm ihn1 ihn2 =>
    ...
    rw[show A[(SStruct.Tm.tt : SStruct.Tm Srt) .: σ] = (A[⇑σ])[(SStruct.Tm.tt : SStruct.Tm Srt)/] by asimp] at ihn1
    rw[show A[(SStruct.Tm.ff : SStruct.Tm Srt) .: σ] = (A[⇑σ])[(SStruct.Tm.ff : SStruct.Tm Srt)/] by asimp] at ihn2
    have er := Extract.ite mrg tyA ihm ihn1 ihn2; asimp at er
    assumption
  case rw A B m m' n a b s i tyA erm tyn ih =>
    have ⟨_, _, _, tyI⟩ := tyA.ctx_inv
    have ⟨_, _, _, tyB⟩ := tyI.ctx_inv
    replace tyA := tyA.substitution ((agr.toLogical.cons tyB).cons tyI)
    rw[show (SStruct.Tm.idn B⟨↑⟩ a⟨↑⟩ (SStruct.Tm.var_Tm 0))[⇑σ]
          = SStruct.Tm.idn ((B[σ])⟨↑⟩) ((a[σ])⟨↑⟩) (SStruct.Tm.var_Tm 0) by asimp] at tyA
    replace erm := ih agr; asimp at erm
    replace tyn := tyn.substitution agr.toLogical; asimp at tyn
    rw[show A[a[σ].rfl .: a[σ] .: σ]
          = (A[⇑⇑σ])[a[σ].rfl, a[σ]/] by asimp] at erm
  ...
  case exf_drop Δ1 Δ2 Δ3 A0 m0 n0 B0 s0 i0 n0' b0' mrg tyA tym ern erb ihn ihb =>
    ...
    rw[show A0[m0[σ] .: σ] = A0[⇑σ][m0[σ]/] by asimp] at erb
  ...
  case drop mrg lw h erm ern ihm ihn =>
    ...
    apply Extract.drop mrg' lw' h ihm ihn
...
lemma Extract.subst_im {Δ : Ctx Srt} {A B m m' n s} :
    A :⟨.im, s⟩ Δ ⊢ m ▷ m' :: B ->
    Δ.logical ⊢ n : A ->
    Δ ⊢ m[n/] ▷ m'[(Extraction.Tm.null : Extraction.Tm Srt)/] :: B[n/] := by
  intro erm tyn
...
lemma Extract.subst_ex {Δ1 Δ2 Δ : Ctx Srt} {A B m m' n n' s} :
...
lemma Extract.esubst_im {Δ : Ctx Srt} {A B1 B2 m1 m2 e1 e2 n s} :
    m2 = m1[n/] ->
    e2 = e1[(Extraction.Tm.null : Extraction.Tm Srt)/] ->
    B2 = B1[n/] ->
    A :⟨.im, s⟩ Δ ⊢ m1 ▷ e1 :: B1 ->
    Δ.logical ⊢ n : A ->
    Δ ⊢ m2 ▷ e2 :: B2 := by
  intros; subst_vars; apply Extract.subst_im <;> assumption

lemma Extract.esubst_ex {Δ1 Δ2 Δ : Ctx Srt} {A B1 B2 m1 e1 m2 e2 n1 n2 s} :
    m2 = m1[n1/] ->
    e2 = e1[n2/] ->
    B2 = B1[n1/] ->
    Lower Δ2 s -> Merge Δ1 Δ2 Δ ->
    ...
    Δ2 ⊢ n1 ▷ n2 :: A ->
    Δ ⊢ m2 ▷ e2 :: B2 := by
  intros; subst_vars; apply Extract.subst_ex <;> assumption

lemma Extract.conv_ctx {Δ : Ctx Srt} {A B C m m' r s i} :
    B === A ->
    Δ.logical ⊢ B : .srt s i ->
    A :⟨r, s⟩ Δ ⊢ m ▷ m' :: C ->
    B :⟨r, s⟩ Δ ⊢ m ▷ m' :: C := by
  ...
  replace erm : B :⟨r, s⟩ Δ ⊢ m[(SStruct.Tm.var_Tm : Var -> SStruct.Tm Srt)] ▷ m'[(Extraction.Tm.var_Tm : Var -> Extraction.Tm Srt)] :: C[(SStruct.Tm.var_Tm : Var -> SStruct.Tm Srt)] := by
    apply erm.substitution
