...
lemma Extract.pi_canonical {A B C m : SStruct.Tm Srt} {m' r s} :
...
lemma Extract.sig_canonical {A B C m : SStruct.Tm Srt} {m' r s} :
  ...
  case tup_im A0 B0 m n n' _ _ _ _ _ _ =>
    have ⟨_, _, _, _⟩ := Logical.Conv.sig_inj eq
...
lemma Extract.bool_canonical {C m : SStruct.Tm Srt} {m'} :
...
theorem Extract.progress {A m} {m' : Tm Srt} :
    [] ⊢ m ▷ m' :: A ->
